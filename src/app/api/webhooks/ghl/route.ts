...
interface GhlWebhookPayload {
  type: string;
  contact: { id: string; email: string; phone?: string; tags?: string[]; customFields?: Record<string, string> };
  status?: string;
}

export async function POST(request: NextRequest) {
  const rawBody = await request.text();
  const signature = request.headers.get("x-ghl-signature") ?? request.headers.get("x-webhook-signature") ?? "";
    ...
    if (mappedStatus) {
      ...
      try {
        const { error } = await supabase
          .from("leads")
          .update({ status: mappedStatus, ghl_synced: true, updated_at: new Date().toISOString() })
