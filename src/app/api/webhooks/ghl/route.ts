...
export async function POST(request: NextRequest) {
  ...
  if (secret) {
    ...
    console.warn(
      "GHL_WEBHOOK_SECRET not set -- skipping signature verification (development mode)",
    );
  }
  ...
  if (payload.type === "ContactStatusChanged" && payload.status) {
    ...
    if (mappedStatus) {
      ...
      try {
        ...
        if (error) {
          console.error(
            "GHL webhook: Failed to update lead status:",
            error,
          );
          return NextResponse.json(
      ...
      console.warn(
        `GHL webhook: Unknown status "${payload.status}", ignoring`,
      );
    }
  }
