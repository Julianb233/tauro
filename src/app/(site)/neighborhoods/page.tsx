...
export default async function NeighborhoodsPage() {
  ...
  return (
    <>
      <section className="relative bg-foreground pt-32 pb-16"><div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/50 to-black/30" /><div className="relative z-10 mx-auto max-w-7xl px-4 text-center sm:px-6 lg:px-8"><p className="font-label text-sm font-semibold uppercase tracking-[0.25em] text-gold">Explore Philadelphia</p><h1 className="mt-3 font-heading text-4xl font-bold text-white sm:text-5xl md:text-6xl">Philadelphia Neighborhoods</h1><p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">From the tree-lined streets of Rittenhouse to the creative energy of Fishtown — discover what makes each neighborhood unique with Tauro&apos;s local expertise.</p></div></section>
      <section className="bg-cream py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{neighborhoods.map((hood) => (<Link key={hood.slug} href={`/neighborhoods/${hood.slug}`} className="group relative overflow-hidden rounded-xl border border-border/40 bg-card transition-all hover:border-gold/40 hover:shadow-xl"><div className="relative aspect-[4/3] overflow-hidden"><Image src={hood.cardImage} alt={hood.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" /></div><div className="absolute inset-x-0 bottom-0 p-5"><div className="flex items-center gap-2"><MapPin className="size-4 text-gold" /><h2 className="font-heading text-xl font-bold text-foreground">{hood.name}</h2></div><p className="mt-1 text-sm text-foreground/70">{hood.tagline}</p><span className="mt-2 inline-block rounded-full bg-gold/20 px-3 py-1 text-xs font-semibold text-gold">Median: {hood.stats.medianPrice}</span></div></Link>))}</div></div></section>
