...
export default async function NeighborhoodsPage() {
  ...
  return (
    <>
      ...
      <section className="bg-cream py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{neighborhoods.map((hood) => (<Link key={hood.slug} href={`/neighborhoods/${hood.slug}`} className="group relative overflow-hidden rounded-xl border border-border/40 bg-card transition-all hover:border-gold/40 hover:shadow-xl hover:border-b-gold hover:border-b-2"><div className="relative aspect-[4/3] overflow-hidden"><Image src={hood.cardImage} alt={hood.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-midnight/90 via-midnight/40 to-transparent" /></div><div className="absolute inset-x-0 bottom-0 p-5"><div className="flex items-center gap-2"><MapPin className="size-4 text-gold" /><h2 className="font-heading text-xl font-bold text-white">{hood.name}</h2></div><p className="mt-1 text-sm text-white/80">{hood.tagline}</p><span className="mt-2 inline-block rounded-full bg-gold/20 px-3 py-1 text-xs font-semibold text-gold">Median: {hood.stats.medianPrice}</span></div></Link>))}</div></div></section>
