...
export function NeighborhoodCompareBar() {
  const { ids, count, remove, clear } = useNeighborhoodCompare();
  ...
  return (
    <div
      role="region"
      aria-label={`Neighborhood comparison: ${count} neighborhoods selected`}
      className="fixed inset-x-0 bottom-0 z-50 border-t border-gold/30 bg-near-black/95 backdrop-blur-md shadow-[0_-4px_24px_rgba(0,0,0,.35)]"
    >
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-4 px-4 py-3">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span
              aria-hidden="true"
              className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-near-black/20 text-xs font-bold"
            >
              {count}
            </span>
            <span className="sr-only">{count} neighborhoods</span>
          </Link>
        </div>
      </div>
    </div>
