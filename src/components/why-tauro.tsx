        ...
        <StaggerReveal className="mt-8 grid gap-4 sm:mt-14 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4 lg:gap-8" stagger={0.12}>
          {whyTauro.map((item) => (
            <TiltCard key={item.title} maxTilt={6}>
              <div className="group glass-card rounded-xl p-6 sm:p-8">
                <div className="icon-ring size-14 rounded-xl">
                  <item.icon className="size-6 text-gold icon-hover-gold" strokeWidth={1.5} />
                </div>
                <h3 className="mt-5 font-heading text-lg font-bold text-white">
                  {item.title}
                </h3>
                <div className="mt-2 h-px w-8 bg-gradient-to-r from-gold/50 to-transparent" />
