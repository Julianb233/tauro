...
const typeColors: Record<string, string> = {
  contact: "bg-blue-500/20 text-blue-400", showing: "bg-emerald-500/20 text-emerald-400",
  seller: "bg-purple-500/20 text-purple-400", "agent-application": "bg-orange-500/20 text-orange-400",
  "agent-contact": "bg-cyan-500/20 text-cyan-400",
};
const statusColors: Record<string, string> = {
  new: "bg-yellow-500/20 text-yellow-400", contacted: "bg-blue-500/20 text-blue-400",
  qualified: "bg-emerald-500/20 text-emerald-400", closed: "bg-white/10 text-white/50",
};

function formatRelativeTime(dateStr: string): string {
  const now = Date.now(); const date = new Date(dateStr).getTime(); const diffMs = now - date;
  ...
  const diffDay = Math.floor(diffHr / 24); if (diffDay < 30) return `${diffDay}d ago`;
